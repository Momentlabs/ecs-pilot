...
const ToolbarTextLogo = ({logoValue, clickOnLogo}) => {
  return (
    <ToolbarGroup>
      <FontIcon className="material-icons" color={color.primary} onClick={clickOnLogo}>directions_boat </FontIcon>
      <ToolbarTitle text={logoValue} style={textStyle} />
    </ToolbarGroup>
  );
};

ToolbarTextLogo.defaultProps = {
  clickOnLogo: undefined
};

ToolbarTextLogo.propTypes = {
  logoValue: PropTypes.string.isRequired,
  clickOnLogo: PropTypes.func
};
